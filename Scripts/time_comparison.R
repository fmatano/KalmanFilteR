...
# setwd("/Users/francescamatano/Dropbox/CMU/thesis/r_code/packages/Stepwise/packages_dependencies/KalmanFilteR/")
...
save_name <- paste0('packages_dependencies/KalmanFilteR/Data/timing_objects_',
                    n_sim,'times.RData')
...
# PLOT THE RESULTS
# Average across simulations, by n_seq
n_units <- 58

extraction_fun <- function(time_list)
  sapply(time_list, function(y) sapply(y, function(x) x[[1]]) %>% mean)


time_ratio <- c((time_sonia_loo_mle %>% extraction_fun)/
                  (time_fra_loo_mle %>% extraction_fun) %>% smooth,
                (time_sonia_loo_kf %>%  extraction_fun)/
                  (time_fra_loo_kf %>%  extraction_fun)%>% smooth)

time_frame <- data.frame(time_ratio = (time_ratio %>% smooth)[1:(length(n_seq)*2)],
                         num_equations = rep(n_seq, times = 2),
                         approach = factor(rep(c("OLE", "KF"),
                                               each = length(n_seq))))
